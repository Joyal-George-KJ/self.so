...
const useTheme = () => {
  ...
  const toggleTheme: (passedTheme?: Theme, toggle?: boolean) => void = (passedTheme: Theme = theme, toggle = false) => {
    if (typeof window !== 'undefined') {
      ...
      if (localStorage.getItem('theme') === null) {
        ...
        let currentTheme: Theme = passedTheme === 'dark' ? 'light' : 'dark';
  ...
  return {mount: mount, theme: theme, toggleTheme: toggleTheme};
};
